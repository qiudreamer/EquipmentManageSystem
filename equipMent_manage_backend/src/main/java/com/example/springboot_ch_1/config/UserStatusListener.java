...
import com.example.springboot_ch_1.entity.msg.UserStatusMsg;
...
public class UserStatusListener implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        try {
            UserStatusMsg msg = mapper.readValue(message.getBody(), UserStatusMsg.class);
            System.out.println("Received status message: " + msg);
            ws.convertAndSend("/topic/userDelete", msg);
        } catch (IOException e) {
            throw new RuntimeException("解析用户状态消息失败", e);
        }
    }
}
